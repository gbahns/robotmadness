import { BoardDefinition, TileType, Direction } from '../types';

export const CROSS_BOARD: BoardDefinition = {
    id: 'cross',
    name: 'Cross',
    width: 12,
    height: 12,

    startingPositions: [
    ],

    tiles: [
        { position: { x: 1, y: 0 }, type: TileType.CONVEYOR, direction: Direction.DOWN },
        { position: { x: 5, y: 0 }, type: TileType.CONVEYOR, direction: Direction.DOWN },
        { position: { x: 6, y: 0 }, type: TileType.CONVEYOR, direction: Direction.UP },
        { position: { x: 10, y: 0 }, type: TileType.CONVEYOR, direction: Direction.UP },
        { position: { x: 1, y: 1 }, entries: [Direction.UP], type: TileType.CONVEYOR, direction: Direction.RIGHT },
        { position: { x: 2, y: 1 }, type: TileType.CONVEYOR, direction: Direction.RIGHT },
        { position: { x: 3, y: 1 }, type: TileType.CONVEYOR, direction: Direction.RIGHT },
        { position: { x: 4, y: 1 }, type: TileType.CONVEYOR, direction: Direction.RIGHT },
        { position: { x: 6, y: 1 }, type: TileType.CONVEYOR, direction: Direction.UP },
        { position: { x: 9, y: 1 }, type: TileType.PIT },
        { position: { x: 10, y: 1 }, entries: [Direction.RIGHT], type: TileType.CONVEYOR, direction: Direction.UP },
        { position: { x: 11, y: 1 }, type: TileType.CONVEYOR, direction: Direction.LEFT },
        { position: { x: 5, y: 2 }, type: TileType.CONVEYOR, direction: Direction.DOWN },
        { position: { x: 6, y: 2 }, type: TileType.CONVEYOR, direction: Direction.UP },
        { position: { x: 11, y: 2 }, type: TileType.REPAIR },
        { position: { x: 2, y: 3 }, type: TileType.PIT },
        { position: { x: 5, y: 3 }, type: TileType.CONVEYOR, direction: Direction.DOWN },
        { position: { x: 6, y: 3 }, type: TileType.CONVEYOR, direction: Direction.UP },
        { position: { x: 5, y: 4 }, type: TileType.CONVEYOR, direction: Direction.DOWN },
        { position: { x: 7, y: 4 }, entries: [Direction.DOWN], type: TileType.CONVEYOR, direction: Direction.LEFT },
        { position: { x: 0, y: 5 }, type: TileType.CONVEYOR, direction: Direction.LEFT },
        { position: { x: 1, y: 5 }, type: TileType.CONVEYOR, direction: Direction.LEFT },
        { position: { x: 2, y: 5 }, type: TileType.CONVEYOR, direction: Direction.LEFT },
        { position: { x: 3, y: 5 }, type: TileType.CONVEYOR, direction: Direction.LEFT },
        { position: { x: 4, y: 5 }, type: TileType.CONVEYOR, direction: Direction.LEFT },
        { position: { x: 5, y: 5 }, entries: [Direction.UP], type: TileType.CONVEYOR, direction: Direction.LEFT },
        { position: { x: 6, y: 5 }, type: TileType.PIT },
        { position: { x: 7, y: 5 }, entries: [Direction.RIGHT], type: TileType.CONVEYOR, direction: Direction.UP },
        { position: { x: 8, y: 5 }, type: TileType.CONVEYOR, direction: Direction.LEFT },
        { position: { x: 9, y: 5 }, type: TileType.CONVEYOR, direction: Direction.LEFT },
        { position: { x: 10, y: 5 }, type: TileType.CONVEYOR, direction: Direction.LEFT },
        { position: { x: 11, y: 5 }, type: TileType.CONVEYOR, direction: Direction.LEFT },
        { position: { x: 0, y: 6 }, type: TileType.CONVEYOR, direction: Direction.RIGHT },
        { position: { x: 1, y: 6 }, type: TileType.CONVEYOR, direction: Direction.RIGHT },
        { position: { x: 2, y: 6 }, type: TileType.CONVEYOR, direction: Direction.RIGHT },
        { position: { x: 3, y: 6 }, type: TileType.CONVEYOR, direction: Direction.RIGHT },
        { position: { x: 5, y: 6 }, type: TileType.PIT },
        { position: { x: 9, y: 6 }, type: TileType.CONVEYOR, direction: Direction.RIGHT },
        { position: { x: 10, y: 6 }, type: TileType.CONVEYOR, direction: Direction.RIGHT },
        { position: { x: 8, y: 7 }, entries: [Direction.LEFT], type: TileType.CONVEYOR, direction: Direction.UP },
        { position: { x: 9, y: 7 }, type: TileType.PIT },
        { position: { x: 5, y: 8 }, type: TileType.CONVEYOR, direction: Direction.DOWN },
        { position: { x: 2, y: 9 }, type: TileType.PIT },
        { position: { x: 5, y: 9 }, type: TileType.CONVEYOR, direction: Direction.DOWN },
        { position: { x: 0, y: 10 }, type: TileType.CONVEYOR, direction: Direction.RIGHT },
        { position: { x: 5, y: 10 }, type: TileType.CONVEYOR, direction: Direction.DOWN },
        { position: { x: 9, y: 10 }, type: TileType.CONVEYOR, direction: Direction.LEFT },
        { position: { x: 0, y: 11 }, type: TileType.REPAIR },
        { position: { x: 5, y: 11 }, type: TileType.CONVEYOR, direction: Direction.DOWN },
        { position: { x: 6, y: 11 }, type: TileType.CONVEYOR, direction: Direction.UP },
        { position: { x: 1, y: 11 }, type: TileType.CONVEYOR, direction: Direction.DOWN },
        { position: { x: 1, y: 10 }, entries: [Direction.LEFT], type: TileType.CONVEYOR, direction: Direction.DOWN },
        { position: { x: 6, y: 9 }, type: TileType.CONVEYOR, direction: Direction.UP },
        { position: { x: 6, y: 6 }, type: TileType.PIT },
        { position: { x: 6, y: 7 }, type: TileType.PIT },
        { position: { x: 7, y: 6 }, type: TileType.PIT },
        { position: { x: 10, y: 7 }, type: TileType.PIT },
        { position: { x: 9, y: 8 }, type: TileType.OPTION },
        { position: { x: 11, y: 6 }, type: TileType.CONVEYOR, direction: Direction.RIGHT },
        { position: { x: 8, y: 6 }, entries: [Direction.DOWN], type: TileType.CONVEYOR, direction: Direction.RIGHT },
        { position: { x: 7, y: 7 }, entries: [Direction.DOWN], type: TileType.CONVEYOR, direction: Direction.RIGHT },
        { position: { x: 7, y: 8 }, entries: [Direction.LEFT], type: TileType.CONVEYOR, direction: Direction.UP },
        { position: { x: 4, y: 7 }, entries: [Direction.UP], type: TileType.CONVEYOR, direction: Direction.RIGHT },
        { position: { x: 5, y: 7 }, entries: [Direction.LEFT], type: TileType.CONVEYOR, direction: Direction.DOWN },
        { position: { x: 4, y: 6 }, entries: [Direction.LEFT], type: TileType.CONVEYOR, direction: Direction.DOWN },
        { position: { x: 6, y: 8 }, entries: [Direction.DOWN], type: TileType.CONVEYOR, direction: Direction.RIGHT },
        { position: { x: 11, y: 0 }, type: TileType.PIT },
        { position: { x: 7, y: 10 }, type: TileType.CONVEYOR, direction: Direction.LEFT },
        { position: { x: 8, y: 10 }, type: TileType.CONVEYOR, direction: Direction.LEFT },
        { position: { x: 10, y: 11 }, type: TileType.CONVEYOR, direction: Direction.UP },
        { position: { x: 10, y: 10 }, entries: [Direction.DOWN], type: TileType.CONVEYOR, direction: Direction.LEFT },
        { position: { x: 2, y: 4 }, type: TileType.OPTION },
        { position: { x: 6, y: 10 }, entries: [Direction.RIGHT, Direction.DOWN], type: TileType.CONVEYOR, direction: Direction.UP },
        { position: { x: 5, y: 1 }, entries: [Direction.LEFT, Direction.UP], type: TileType.CONVEYOR, direction: Direction.DOWN },
        { position: { x: 6, y: 4 }, entries: [Direction.RIGHT], type: TileType.CONVEYOR, direction: Direction.UP },
    ],

    lasers: [
        { position: { x: 3, y: 8 }, direction: Direction.DOWN, damage: 2 },
        { position: { x: 9, y: 3 }, direction: Direction.LEFT, damage: 1 },
        { position: { x: 7, y: 9 }, direction: Direction.DOWN, damage: 1 },
        { position: { x: 4, y: 3 }, direction: Direction.LEFT, damage: 1 },
    ],

    walls: [
        { position: { x: 7, y: 3 }, sides: [Direction.RIGHT, Direction.DOWN] },
        { position: { x: 4, y: 8 }, sides: [Direction.UP, Direction.RIGHT] },
        { position: { x: 4, y: 11 }, sides: [Direction.DOWN] },
        { position: { x: 9, y: 0 }, sides: [Direction.UP] },
        { position: { x: 7, y: 0 }, sides: [Direction.UP] },
        { position: { x: 4, y: 0 }, sides: [Direction.UP, Direction.LEFT] },
        { position: { x: 0, y: 2 }, sides: [Direction.LEFT] },
        { position: { x: 10, y: 8 }, sides: [Direction.LEFT, Direction.UP] },
        { position: { x: 11, y: 7 }, sides: [Direction.RIGHT] },
        { position: { x: 11, y: 9 }, sides: [Direction.RIGHT] },
        { position: { x: 7, y: 11 }, sides: [Direction.DOWN] },
        { position: { x: 9, y: 11 }, sides: [Direction.DOWN] },
        { position: { x: 2, y: 0 }, sides: [Direction.UP] },
        { position: { x: 4, y: 4 }, sides: [Direction.DOWN, Direction.RIGHT] },
        { position: { x: 4, y: 3 }, sides: [Direction.RIGHT] },
        { position: { x: 11, y: 4 }, sides: [Direction.UP, Direction.RIGHT] },
        { position: { x: 9, y: 2 }, sides: [Direction.LEFT] },
        { position: { x: 3, y: 3 }, sides: [Direction.LEFT] },
        { position: { x: 0, y: 4 }, sides: [Direction.LEFT] },
        { position: { x: 2, y: 7 }, sides: [Direction.UP] },
        { position: { x: 3, y: 8 }, sides: [Direction.UP] },
        { position: { x: 3, y: 11 }, sides: [Direction.UP] },
        { position: { x: 2, y: 11 }, sides: [Direction.DOWN] },
        { position: { x: 7, y: 9 }, sides: [Direction.UP] },
        { position: { x: 8, y: 8 }, sides: [Direction.LEFT] },
        { position: { x: 9, y: 3 }, sides: [Direction.RIGHT] },
        { position: { x: 11, y: 2 }, sides: [Direction.RIGHT] },
        { position: { x: 0, y: 9 }, sides: [Direction.LEFT] },
        { position: { x: 0, y: 7 }, sides: [Direction.LEFT] },
        { position: { x: 1, y: 4 }, sides: [Direction.DOWN] },
    ],
};