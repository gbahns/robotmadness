...
export interface OptionCard {
  ...
  damageValue?: number;
  implemented?: boolean;
  passive?: boolean; // True if the card's effect is always active
  damageAbsorbed?: number; // For Ablative Coat tracking
  usedThisRegister?: boolean; // For Shield/Power-Down Shield tracking per register
  directionsUsed?: number[]; // For Power-Down Shield tracking which directions have blocked damage
}
...
export const OPTION_CARDS: Record<OptionCardType, Omit<OptionCard, 'id'>> = {
  ...
  [OptionCardType.RAMMING_GEAR]: {
    ...
    name: 'Ramming Gear',
    description: 'Whenever your robot pushes another robot, that robot receives 1 Damage.',
    implemented: true,
    passive: true
  },
  ...
  [OptionCardType.SHIELD]: {
    ...
    name: 'Shield',
    description: 'Each register phase, your robot takes no Damage from the first robot to shoot at it from the front this phase. Can be discarded to prevent 1 damage from any direction.',
    implemented: true,
    passive: true
  },
...
export function createOptionCard(type: OptionCardType): OptionCard {
  ...
  return {
    ...
    // All option cards can be discarded to prevent 1 damage (unless they have a specific value)
    damageValue: template.damageValue || 1
  };
}
...
export function createOptionDeck(): OptionCard[] {
  ...
  Object.values(OptionCardType).forEach(type => {
    // Exclude the custom Shield card from standard deck
    // Shield is a custom card that only blocks from front - keep in reserve
